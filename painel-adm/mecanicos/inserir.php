...
if($antigo != $cpf){
	$query = $pdo->query("SELECT * FROM mecanicos where cpf = '$cpf' and id_oficina = '$id_oficina'");
...
if($antigo2 != $email){
	$query = $pdo->query("SELECT * FROM mecanicos where email = '$email' and id_oficina = '$id_oficina'");
	$res = $query->fetchAll(PDO::FETCH_ASSOC);
	$total_reg = @count($res);
	if($total_reg > 0){
...
if($id == ""){
	$res = $pdo->prepare("INSERT INTO mecanicos SET nome = :nome, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone, id_oficina = :id_oficina");	
	...
	$res = $pdo->prepare("UPDATE mecanicos SET nome = :nome, cpf = :cpf, email = :email, endereco = :endereco, telefone = :telefone, id_oficina = :id_oficina WHERE id = '$id' and id_oficina = '$id_oficina'");
