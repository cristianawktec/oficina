...
$query_cat = $pdo->query("SELECT * FROM os where mecanico = '$_SESSION[cpf_usuario]' and concluido = 'Sim' and id_oficina = '$id_oficina'");
...
$query_cat = $pdo->query("SELECT * FROM comissoes where data = curDate() and mecanico = '$_SESSION[cpf_usuario]' and id_oficina = $id_oficina");
$res_cat = $query_cat->fetchAll(PDO::FETCH_ASSOC);
...
$totalComissoesMes = 0;
$query_cat = $pdo->query("SELECT * FROM comissoes where data >= '$dataInicioMes' and data <= curDate() and mecanico = '$_SESSION[cpf_usuario]' and id_oficina = $id_oficina");
$res_cat = $query_cat->fetchAll(PDO::FETCH_ASSOC);
for ($i=0; $i < @count($res_cat); $i++) { 
	foreach ($res_cat[$i] as $key => $value) {
	}
...
<div class="row">
	...
	$query_cat = $pdo->query("SELECT * FROM os where concluido != 'Sim' and mecanico = '$_SESSION[cpf_usuario]' and id_oficina = $id_oficina order by data_entrega asc, id asc limit 12");
	$res_cat = $query_cat->fetchAll(PDO::FETCH_ASSOC);
	for ($i=0; $i < @count($res_cat); $i++) { 
		foreach ($res_cat[$i] as $key => $value) {
		}
		...
		$query = $pdo->query("SELECT * FROM veiculos where id = '$veiculo' and id_oficina = $id_oficina");
